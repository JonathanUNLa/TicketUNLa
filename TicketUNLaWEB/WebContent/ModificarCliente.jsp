...
<!--==============================header=================================-->
		...
		alta clientes
			...
			<%List<Usuario> user = (List) request.getAttribute("usuario");%>
			
				
				
				<FORM method="POST" action="/TicketUNLaWEB/ClienteModificado">
				
					<%for (Usuario u: user){ %>
					<p>	<input type="radio" id="idUsuario" name="idUsuario" value="<%= u.getIdUsuario() %>" checked>
						Dni: <%= u.getDni() %> Apellido: <%= u.getApellido() %> Nombre: <%= u.getNombre() %> Telefono: <%= u.getTelefono() %><br>
						Nombre de Usuario <%= u.getNombreUsuario() %> Contraseña: <%= u.getContraseña() %> Tipo de usuario <%= u.getTipoUsuario().getNombreTipo() %><br>
						Tipo beneficio:
						 <%if(u.getTipoBeneficio()!=null){ %>
						 <%= u.getTipoBeneficio().getBeneficio()%>
						 <%} %>
					</p>
					<% } %>
					...
					<input type="text" name="nombreUsuario" placeholder="Ingrese nombre de usuario" ><br>
