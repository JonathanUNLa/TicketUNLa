...
<%@page import="datos.DiaDescuento"%>
...
<!--==============================header=================================-->
		...
		<section id="content">
				<div class="grid_8">
					alta funciones
			<%List<Funcion> funcion = (List) request.getAttribute("funcion");%>
			...
			<%List<DiaDescuento> diasDescuentos = (List) request.getAttribute("diasDescuentos");%>
					<FORM method="POST" action="/TicketUNLaWEB/FuncionModificada">
					
					<select name="idFuncion">
					<%for(Funcion f: funcion){ %>
						<option value="<%= f.getIdFuncion() %>"> <%= Funciones.traerFechaCortaHora(f.getDiaHora()) %></option>			
						<%}%>			
					</select> 
