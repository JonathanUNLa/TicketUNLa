...
<!--==============================header=================================-->
		...
		<div class="grid_8">
		alta Codigo de Descuentos
			
			<%List<Seccion> secciones = (List) request.getAttribute("secciones");%>
			<%List<Funcion> funciones = (List) request.getAttribute("funciones");%>
					<FORM method="POST" action="/TicketUNLaWEB/CrearCliente">
					<input type="text" name="codigo" placeholder="Ingrese Codigo" > <br>
					<input type="text" name="descuento" placeholder="Ingrese Porcentaje Descuento" ><br>				
					
					<select name="secciones">
					<option value="9999">  </option>
					<%for(Seccion s: secciones){ %>
						<option value="<%= s.getIdSeccion() %>"> <%= s.getNombreSeccion() %></option>			
						<%}%>			
					</select> 
					...
					<%for(Funcion f: funciones){ %>
						<option value="<%= f.getIdFuncion() %>"> <%= f.getDiaHora() %></option>			
						<%}%>			
					</select> 	
