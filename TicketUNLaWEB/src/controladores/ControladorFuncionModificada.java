...
public class ControladorFuncionModificada extends HttpServlet{
	...
	private void procesarPeticion(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			Funcion fun= aabm.traerFuncion(idFuncion);
			fun.setEvento(evento);
			fun.setDiaDescuento(diaDescuento);
			fun.setDiaHora(fechaHora);
			fun.setPrecioBase(precioBase);
			
			aabm.modificar(fun);
			String respuesta= "La funcion fue modificada con exito";
			
			request.setAttribute("respuesta",respuesta);
			request.getRequestDispatcher("/FuncionModificada.jsp").forward(request , response);
		}
		catch (Exception e) {
			if(error==0){
