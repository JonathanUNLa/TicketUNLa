...
public class ControladorTicketFechaEventoCliente extends HttpServlet{
	...
	private void procesarPeticion(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			List<Usuario> lista = uabm.traerUsuario();
			request.setAttribute("idevento",idevento);
			request.setAttribute("usuario",lista);
			
			request.getRequestDispatcher("/TicketFechaEventoCliente.jsp").forward(request , response);
		}
		catch (Exception e) {
			if(error==0){
