...
public class ControladorBajaauditorios extends HttpServlet{
	...
	private void procesarPeticion(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			List<Auditorio> la = auditorioAbm.traerAuditorio();
			
			request.setAttribute("la",la);
			request.getRequestDispatcher("/bajaauditorios.jsp").forward(request , response);
		}
		catch (Exception e) {
			if(error==0){
