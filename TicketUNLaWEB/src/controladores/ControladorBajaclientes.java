...
public class ControladorBajaclientes extends HttpServlet{
	...
	private void procesarPeticion(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			List<Usuario> lu = usuarioAbm.traerUsuario();
			
			request.setAttribute("lu",lu);
			request.getRequestDispatcher("/bajaclientes.jsp").forward(request , response);
		}
		catch (Exception e) {
			if(error==0){
