...
public class ControladorTicketNoReservadoEvento extends HttpServlet {
	...
	private void procesarPeticion(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			List<Funcion>funciones= fabm.traerFuncionEvento(idEvento);
			int idAuditorio= evabm.traerEventoHql(idEvento).getAuditorio().getIdAuditorio();
		
			List<Butaca> butacasDevolver= new ArrayList<Butaca>();
			
			for(Funcion f: funciones) {
					int x=0;
					List<Butaca> tbutacas= babm.traerButacaAuditorio(idAuditorio);
					List<Butaca> bt= eabm.traerEntradaFuncion(f.getIdFuncion());
					while(x< bt.size()) {
						tbutacas.remove(bt.get(x));
						x++;
					}
					for(Butaca b:tbutacas) {
						butacasDevolver.add(b);
					}
		...
		/*	List<Funcion> funciones = fabm.traerFuncion(evabm.traerEvento(idEvento).getNombre());
			...
			String evento= evabm.traerEvento(idEvento).getNombre();
			request.setAttribute("evento",evento);
			request.setAttribute("butacasDevolver",butacasDevolver);
